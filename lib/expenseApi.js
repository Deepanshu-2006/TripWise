 ...
 */
export const getUserDisplayCurrency = (nativeCost = null) => {
  if (typeof window === 'undefined') return 'USD';
  
  try {
    // Default to the native currency of the trip if provided (e.g. ¥ for Japan)
    if (nativeCost) {
      const str = String(nativeCost);
    ...
    // Otherwise, check user's OS Language/Region Nationality setting
...
/**
 * Safely parse and convert estimated cost strings to a numeric value in the user's currency
 */
export const getConvertedEstimatedCostNumber = (costStr, targetCurrency = 'USD') => {
  const str = String(costStr || '$1,450');
  let sourceCurrency = 'USD';
  
  if (str.includes('€')) sourceCurrency = 'EUR';
  else if (str.includes('£')) sourceCurrency = 'GBP';
  else if (str.includes('¥')) sourceCurrency = 'JPY';
  else if (str.includes('₹')) sourceCurrency = 'INR';
  else if (str.includes('A$')) sourceCurrency = 'AUD';
  else if (str.includes('CA$')) sourceCurrency = 'CAD';
  else if (str.includes('CHF')) sourceCurrency = 'CHF';

  // Extract the first valid number
  const match = str.match(/[\d,]+(\.\d+)?/);
  const num = match ? parseFloat(match[0].replace(/,/g, '')) : 1450;
  
  return convertCurrency(num, sourceCurrency, targetCurrency);
};

/**
 * Safely parse, convert, and format estimated cost strings to user's currency
 */
export const getConvertedEstimatedCost = (costStr, targetCurrency = 'USD') => {
  const converted = getConvertedEstimatedCostNumber(costStr, targetCurrency);
