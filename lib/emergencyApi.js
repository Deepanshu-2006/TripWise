...
// City / Region to Country Mapping Database
...
/**
 * Resolves a destination string to its canonical country
 */
function resolveDestinationCountry(destStr) {
  if (!destStr) return 'United Kingdom';
  const clean = destStr.toLowerCase().trim();

  // 1. Direct match with EMERGENCY_NUMBERS keys
  const directMatch = Object.keys(EMERGENCY_NUMBERS).find(
    k => clean === k.toLowerCase()
  );
  if (directMatch) return directMatch;

  // 2. Direct city/alias lookup
  for (const [cityKey, countryVal] of Object.entries(CITY_TO_COUNTRY)) {
    if (clean === cityKey || clean.includes(cityKey)) {
      return countryVal;
    }
  }

  // 3. Substring match with country keys
  const subMatch = Object.keys(EMERGENCY_NUMBERS).find(
    k => clean.includes(k.toLowerCase()) || k.toLowerCase().includes(clean)
  );
  if (subMatch) return subMatch;

  // 4. Token-level match (e.g., "London, UK" -> ["london", "uk"])
  const tokens = clean.split(/[,\s/-]+/).filter(Boolean);
  for (const t of tokens) {
    if (CITY_TO_COUNTRY[t]) return CITY_TO_COUNTRY[t];
    const country = Object.keys(EMERGENCY_NUMBERS).find(k => k.toLowerCase() === t);
    if (country) return country;
  }

  // 5. Fallback to extracting the trailing country or destination name
  const parts = destStr.split(',');
  return parts.length > 1 ? parts[parts.length - 1].trim() : destStr.trim();
}
...
export async function fetchEmergencyInfo(nationalityCode, destinationName, userCoords = null) {
  ...
  const countryKey = resolveDestinationCountry(cleanDest);
  ...
  const numbersData = EMERGENCY_NUMBERS[countryKey] || {
    ...
    ambulance: '112',
    universal: '112',
    note: `Universal Emergency Helpline for ${countryKey || cleanDest}`
  };
  ...
  let embassyResult = { 
    coverage: false, 
    data: null, 
    message: `Diplomatic mission details for ${countryKey || cleanDest} are available on Google Maps.` 
  };
  const upperNat = (nationalityCode || '').toUpperCase();

  if (upperNat && EMBASSY_DIRECTORY[upperNat]) {
    const natEmbassies = EMBASSY_DIRECTORY[upperNat];
    
    // Check by countryKey first, then cleanDest
    const destKey = Object.keys(natEmbassies).find(
      k => (countryKey && k.toLowerCase() === countryKey.toLowerCase()) ||
           cleanDest.toLowerCase().includes(k.toLowerCase()) || 
           k.toLowerCase().includes(cleanDest.toLowerCase())
    );

    if (destKey && natEmbassies[destKey]) {
      embassyResult = {
        coverage: true,
        data: natEmbassies[destKey],
        message: null
      };
    }
  }
  ...
  if (cityKey && DESTINATION_HOSPITALS[cityKey]) {
    ...
    const anchorLat = userCoords?.lat || (cityKey === 'London' ? 51.5074 : cityKey === 'Rome' ? 41.9028 : cityKey === 'Tokyo' ? 35.6762 : cityKey === 'Paris' ? 48.8566 : cityKey === 'Dubai' ? 25.2048 : 40.7128);
    const anchorLng = userCoords?.lng || (cityKey === 'London' ? -0.1278 : cityKey === 'Rome' ? 12.4964 : cityKey === 'Tokyo' ? 139.6503 : cityKey === 'Paris' ? 2.3522 : cityKey === 'Dubai' ? 55.2708 : -74.0060);
    ...
    const fallbackMapUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent('hospital emergency 24/7 ' + cleanDest)}`;
    hospitalsList = [
      {
        ...
        traumaCenter: true,
        distanceKm: 1.2,
        distanceStr: '~1.2 km from center',
