...
export async function fetchVisaRequirements(nationalityCode, destinationName) {
  ...
  const fallbackResponse = {
    coverage: true,
    data: {
      required: 'Check Official Requirements',
      details: `Please verify specific visa requirements for entering ${destinationName} based on your passport (${upperNat}).`,
      processingTime: 'Varies by embassy',
      passportValidity: 'Generally must be valid for at least 6 months from the date of entry.',
      proofOfOnwardTravel: true,
      minimumFunds: 'Proof of sufficient funds may be required.',
      embassyLink: `https://www.google.com/search?q=${encodeURIComponent(destinationName)}+visa+requirements+for+${encodeURIComponent(upperNat)}+citizens`,
    }
  };
