...
import { buildSkyscannerFlightUrl, getAirportDetails, getIataCode } from './iataCodes.js';
...
export const searchFlights = async (destinationName, origin, dates) => {
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      const originAirport = getAirportDetails(origin);
      const destIata = getIataCode(destinationName);
      const destAirport = getAirportDetails(destIata);
      
      const isIndia = (originAirport?.country === 'India') || (destAirport?.country === 'India') || destinationName.toLowerCase().includes('india') || destinationName.toLowerCase().includes('punjab');
      const isEurope = (originAirport?.region === 'Europe') || (destAirport?.region === 'Europe');
      
      let airlineData = [];
      let layovers = [];
      let isShortHaul = false;
      
      if (isIndia) {
        airlineData = [
          { code: '6E', airline: 'IndiGo', logo: 'https://images.kiwi.com/airlines/64/6E.png' },
          { code: 'AI', airline: 'Air India', logo: 'https://images.kiwi.com/airlines/64/AI.png' },
          { code: 'UK', airline: 'Vistara', logo: 'https://images.kiwi.com/airlines/64/UK.png' },
          { code: 'SG', airline: 'SpiceJet', logo: 'https://images.kiwi.com/airlines/64/SG.png' },
          { code: 'QP', airline: 'Akasa Air', logo: 'https://images.kiwi.com/airlines/64/QP.png' },
          { code: 'I5', airline: 'AIX Connect', logo: 'https://images.kiwi.com/airlines/64/I5.png' }
        ];
        layovers = ['DEL', 'BOM', 'BLR', 'HYD', 'CCU', 'AMD'];
        isShortHaul = (originAirport?.country === 'India' && (destAirport?.country === 'India' || destinationName.toLowerCase().includes('india') || destinationName.toLowerCase().includes('punjab')));
      } else if (isEurope) {
        airlineData = [
          ...
          { code: 'KL', airline: 'KLM', logo: 'https://images.kiwi.com/airlines/64/KL.png' },
          { code: 'FR', airline: 'Ryanair', logo: 'https://images.kiwi.com/airlines/64/FR.png' },
          { code: 'U2', airline: 'easyJet', logo: 'https://images.kiwi.com/airlines/64/U2.png' }
        ];
        layovers = ['LHR', 'CDG', 'FRA', 'AMS', 'MAD', 'MUC'];
        isShortHaul = (originAirport?.region === 'Europe' && destAirport?.region === 'Europe');
      } else {
        airlineData = [
        ...
        layovers = ['JFK', 'ORD', 'FRA', 'CDG', 'LHR', 'AMS', 'DXB', 'MUC'];
        isShortHaul = false;
      }

      const timeSlots = [
        { dep: '06:15 AM', arr: isShortHaul ? '08:45 AM' : '02:45 PM' },
        { dep: '08:30 AM', arr: isShortHaul ? '11:15 AM' : '04:15 PM' },
        { dep: '10:45 AM', arr: isShortHaul ? '01:20 PM' : '07:20 PM' },
        { dep: '01:15 PM', arr: isShortHaul ? '03:50 PM' : '09:50 PM' },
        { dep: '03:40 PM', arr: isShortHaul ? '06:10 PM' : '11:10 PM' },
        { dep: '06:20 PM', arr: isShortHaul ? '08:45 PM' : '06:45 AM (+1)' },
        { dep: '08:50 PM', arr: isShortHaul ? '11:15 PM' : '09:15 AM (+1)' },
        { dep: '11:10 PM', arr: isShortHaul ? '01:30 AM (+1)' : '12:30 PM (+1)' }
      ];
      ...
      const results = Array.from({ length: 8 }).map((_, i) => {
        ...
        let durationHours;
        if (isShortHaul) {
          durationHours = isNonstop ? (1 + (i % 2)) : (3 + (i % 2));
        } else {
          durationHours = isNonstop ? (7 + (i % 4)) : (11 + (i % 5));
        }
        ...
        let finalPrice = Math.round(basePrice * (0.75 + (i * 0.08) - (isNonstop ? 0 : 0.12)));
        if (isShortHaul) {
          finalPrice = Math.round(finalPrice * 0.25); // Domestic flights are usually cheaper
        }
    ...
    setTimeout(async () => {
      ...
      if (hotelList.length === 0) {
        hotelList = REAL_DESTINATION_HOTELS[destKey] || [
          { name: `Grand ${destShort} Palace`, rating: 5, distance: '0.3 mi from city center', image: 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=600&h=400&fit=crop' },
          { name: `${destShort} Boutique Hotel`, rating: 4, distance: '0.5 mi from city center', image: 'https://images.unsplash.com/photo-1582719478250-c89cae4dc85b?w=600&h=400&fit=crop' },
          { name: `The Heritage ${destShort} Suites`, rating: 5, distance: '0.8 mi from city center', image: 'https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?w=600&h=400&fit=crop' },
          { name: `Royal ${destShort} Plaza`, rating: 4, distance: '0.4 mi from city center', image: 'https://images.unsplash.com/photo-1571896349842-33c89424de2d?w=600&h=400&fit=crop' },
          { name: `Continental ${destShort} Resort`, rating: 5, distance: '1.2 mi from city center', image: 'https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?w=600&h=400&fit=crop' },
          { name: `The Inn at ${destShort}`, rating: 4, distance: '0.6 mi from city center', image: 'https://images.unsplash.com/photo-1590490360182-c33d57733427?w=600&h=400&fit=crop' },
          { name: `Crown ${destShort} Center Hotel`, rating: 4, distance: '1.0 mi from city center', image: 'https://images.unsplash.com/photo-1566665797739-1674de7a421a?w=600&h=400&fit=crop' },
          { name: `Apex ${destShort} Vista Inn`, rating: 4, distance: '1.4 mi from city center', image: 'https://images.unsplash.com/photo-1618773928121-c32242e63f39?w=600&h=400&fit=crop' }
        ];
      }
