...
export const searchHotels = async (destinationName, dates) => {
  return new Promise((resolve) => {
    setTimeout(async () => {
      ...
      const realList = REAL_DESTINATION_HOTELS[destKey] || [
      ...
      const results = await Promise.all(realList.map(async (hotel, i) => {
        ...
        let realPhoto = null;
        try {
          // Fetch real photo from Google Places API route
