...
export function generateICS(itinerary, options = {}) {
  ...
  const destination = itinerary.destination || 'My Trip';
  ...
  if (days.length > 0) {
    ...
    lines.push(foldLine(`SUMMARY:✈️ Trip to ${escapeICS(destination)}`));
    lines.push(foldLine(`DESCRIPTION:${escapeICS(days.length)} day trip planned with TripWise AI. ${escapeICS(itinerary.tagline || '')}`));
...
export function getGoogleCalendarTripUrl(itinerary) {
  ...
  const title = encodeURIComponent(`✈️ Trip to ${destination}`);
  const details = encodeURIComponent(
    `${days.length} day trip planned with TripWise AI.\n${itinerary.tagline || ''}`
  );
